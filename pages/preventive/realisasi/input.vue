<template>
  <div class="animate-fade-in">
    <div class="flex justify-between items-center mb-6">
      ...
      <NuxtLink to="/preventive/realisasi" class="btn btn-secondary">
        ← Kembali
      </NuxtLink>
    </div>
...
onMounted(async () => {
  if (isEdit.value && editId.value) {
    ...
    try {
      const data = await $fetch(`/api/pm/realizations/${editId.value}`)
...
const loadMaterials = async () => {
  ...
  try {
    const response = await $fetch(`/api/pm/materials/${form.unit}`)
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await $fetch(`/api/pm/realizations/${editId.value}`, {
        method: 'PUT',
        body: payload
      })
    } else {
      await $fetch('/api/pm/realizations', {
        method: 'POST',
        body: payload
      })
    }
